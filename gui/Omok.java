...
class MyPanel extends JPanel implements MouseListener, ActionListener {
	...
	void judge() {
		...
		for (int y = 0; y < MAX_SIZE; y++) {
			...
			for (int x = 0; x < MAX_SIZE - 4; x++) {
				...
				if (grid[y][x].type == 1 && grid[y][x + 1].type == 1 && grid[y][x + 2].type == 1
						&& grid[y][x + 3].type == 1 && grid[y][x + 4].type == 1) {

					win = 1;

				}

				if (grid[y][x].type == 2 && grid[y][x + 1].type == 2 && grid[y][x + 2].type == 2
						&& grid[y][x + 3].type == 2 && grid[y][x + 4].type == 2) {
		...
		for (int y = 0; y < MAX_SIZE - 4; y++) {
			...
			for (int x = 0; x < MAX_SIZE; x++) {
				...
				if (grid[y][x].type == 1 && grid[y + 1][x].type == 1 && grid[y + 2][x].type == 1
						&& grid[y + 3][x].type == 1 && grid[y + 4][x].type == 1) {

					win = 1;

				}

				if (grid[y][x].type == 2 && grid[y + 1][x].type == 2 && grid[y + 2][x].type == 2
						&& grid[y + 3][x].type == 2 && grid[y + 4][x].type == 2) {
		...
		for (int y = 0; y < MAX_SIZE - 4; y++) {
			...
			for (int x = 0; x < MAX_SIZE - 4; x++) {
				...
				if (grid[y][x].type == 1 && grid[y + 1][x + 1].type == 1 && grid[y + 2][x + 2].type == 1
						&& grid[y + 3][x + 3].type == 1 && grid[y + 4][x + 4].type == 1) {

					win = 1;

				}

				if (grid[y][x].type == 2 && grid[y + 1][x + 1].type == 2 && grid[y + 2][x + 2].type == 2
						&& grid[y + 3][x + 3].type == 2 && grid[y + 4][x + 4].type == 2) {
	...
	protected void paintComponent(Graphics g) {
		...
		g.drawString("오목게임", 50, 50);
		g.setFont(new Font("",Font.PLAIN, 20));
		g.drawString("Player 1 : ○", 300, 50);
		g.drawString("Player 2 : ●", 500, 50);

		
		if (stoneType) {

			g.drawString("Player 1 (○) 차례", 50, 100);

		} else {

			g.drawString("Player 2 (●) 차례", 50, 100);
		...
		if (win == 1) {
			...
			g.drawString("Player 1 승리!", 50, 150);

			end = true;

		} else if (win == 2) {

			g.drawString("Player 2 승리!", 50, 150);
