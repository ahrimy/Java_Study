...
public class Join_Panel extends JPanel implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == back_button) {
			...
			if (name_tf.getText().equals("") || age_tf.getText().equals("") || mobile_tf.getText().equals("")
					|| id_tf.getText().equals("") || pw_tf.getText().equals("")) {
				JOptionPane.showMessageDialog(null, "모든필드를 입력하세요", "빈 필드 존재", JOptionPane.WARNING_MESSAGE);
				return;
			}
			if (FileManager.instance.checkID(id_tf.getText())) {
				JOptionPane.showMessageDialog(null, "이미 존재하는 아이디 입니다.", "중복 ID", JOptionPane.WARNING_MESSAGE);
				return;
			}
			...
			JOptionPane.showMessageDialog(null, "회원가입이 완료되었습니다.", "가입 완료", JOptionPane.CLOSED_OPTION);
			reset();
		}
	}
